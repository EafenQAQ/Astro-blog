    ...
    <div id="SearchResult">
        ...
        <div>
           <p>
            ...
            搜索「{{searchContent}}」，结果如下：</p>
        </div>
        <div class=" min-h-screen h-fit">
            <div v-if="searchResults.length === 0" class="flex justify-center items-center">
                <p>您搜索的内容「{{ searchContent }}」, 没有搜索结果</p>
            </div>
            <template v-else>
                <div v-for="post in searchResults">
                    <ul
                        class="grid grid-cols-2 grid-rows-3 gap-6 2xl:grid-cols-3 justify-items-center max-sm:grid-cols-1">
      ...
      rounded-2xl p-5">
                            <a :href="post.url" class="hover:cursor-pointer">
                                ...
                                <div class="flex flex-col gap-2">
                                    <h2 class="text-2xl max-sm:text-xl font-semibold max-sm:pb-5">
