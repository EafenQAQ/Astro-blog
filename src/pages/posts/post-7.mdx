...
- **快速拉取镜像并启动服务**
  ...
  **Docker 方案**：
  ...
  没错，你已经起了一个密码是 123456 的 mysql 容器了，怎么样，docker 很神奇吧！

- **易保存、迁移**
  - 可以打包成 `.tar` 文件到处传播。
  - 可以打上标签上传到 Docker Hub 到处传播。
...
TUN 会模拟出一张虚拟网卡，所有流量都会经过这张网卡，愿不愿意，可由不得你了
...
启用后，Docker Server 会在 `127.0.0.1:2375` 暴露一个无 TLS 加密的端口。这样，只要能够让别的机器访问到 windows 的 localhost:2375 就可以使用 docker 服务了。这种事情，真的做的到吗？
...
<Quiz client:idle
  title="问题 1 Docker的好处是什么？"
  quizId="q1"
  options={[
    { label: "1145141919810！", value: "option1" },
    { label: "一次配置，到处运行！", value: "option2" },
    { label: "没有好处", value: "option3" },
    { label: "是一个一个一个好的docker！", value: "option4" },
  ]}
